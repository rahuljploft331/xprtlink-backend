...
export async function getById(req, res, next) {
  try {
    ...
    if (!r) return res.status(404).json({ success: false, message: "Not found", code: "NOT_FOUND" });
...
export async function hide(req, res, next) {
  try {
    ...
    return ResponseFormatter.success(res, { message: "Review hidden", data: r });
  } catch (err) { next(err); }
}
...
export async function publish(req, res, next) {
  try {
    ...
    return ResponseFormatter.success(res, { message: "Review published", data: r });
  } catch (err) { next(err); }
}
